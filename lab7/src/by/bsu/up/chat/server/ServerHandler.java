...
public class ServerHandler implements HttpHandler {
    ...
    private Response doPut(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.parsePutRequest(httpExchange.getRequestBody());
            String id = message.getId();
            String text = message.getText();
            logger.info(String.format("Received new update from user id: %s, text: %s", id, text));
            if(messageStorage.updateMessage(id, text)){
                logger.info("Update msg with id: " + id);
                return Response.ok();
            } else {
                return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect msg id");
            }
        } catch (ParseException e) {
            logger.error("Could not parse message.", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
    ...
    private Response doDelete(HttpExchange httpExchange) {
        try {
            String id = MessageHelper.parseDeleteRequest(httpExchange.getRequestBody());
            logger.info(String.format("Received new remove from user id: %s", id));
            if(messageStorage.removeMessage(id)){
                logger.info("Remove msg with id: " + id);
                return Response.ok();
            } else {
                return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect msg id");
            }
